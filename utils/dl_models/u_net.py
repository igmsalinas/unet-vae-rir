...
class UNet:
    ...
    def vector_block(self, input_v, block_shape):
        ...
        x = Dense(dim * 2, name="encoder_inf_dense")(x)
        x = Dense(dim, name="encoder_inf_dense_2")(x)
        x = Dropout(.3)(x)
        x = Reshape(shape)(x)
        x = Conv2D(self.number_filters_0 * 2, (1, 1))(x)
        x = Conv2D(self.number_filters_0 * 4, (1, 1))(x)
